...
% MONITOR_AND_INGEST_CORE: The core logic that runs every cycle.
    ...
    % NOTE: This assumes ProjectFusion_Learning_Listener is defined elsewhere 
    % in a file accessible to the MATLAB path.
    listener_function = @ProjectFusion_Learning_Listener;
    
    % --- 1. Define the List of Files to Monitor and Their Target URLs (5 DOCUMENTS) ---
    % These links represent the URLs provided by the Cloud API (OneDrive/Google Drive).
    files_to_monitor = {
        % 1. Core Physics Document (LIVE URL)
        ...
        % 2. Financial Metrics Spreadsheet (LIVE URL)
        ...
        % 3. ADAS Functional Safety Document (SIMULATED URL - Requires Live Update)
        struct('name', 'ADAS-Smart_Clear-TPJLR-12-027 Copy.pdf', ...
               'url', 'https://googledrive.com/file/adas_tpjlr_12_027.pdf', ...
               'type', 'Functional_Safety_Data'),
               
        % 4. Manufacturing Quotation (SIMULATED URL - Requires Live Update)
        struct('name', 'Quotation_Request_SEC_S400_ZL_Vacuum Copy.pdf', ...
               'url', 'https://onedrive.live.com/file/SEC_S400_quotation_final.pdf', ...
               'type', 'Manufacturing_Logistics'),
               
        % 5. Solar Prototype Bill of Materials (SIMULATED URL - Requires Live Update)
        struct('name', 'Apollon_Grid_Solar_Prototype_IPCH02S5000.RB_Review28.10.25docx_053156[Prince].docx', ...
               'url', 'https://googledrive.com/file/apollon_prototype_bom.docx', ...
    ...
    for i = 1:length(files_to_monitor)
        ...
        % FIX: Ensure Files 1-4 are always checked (live/high priority) to push status updates.
        % File 5 uses a random check to simulate dynamic detection of new files.
        if i <= 4 || rand() > 0.5
...
% --------------------------------------------------------------------------
% --- REQUIRED EXTERNAL FUNCTION (Must be in a separate file: ProjectFusion_Learning_Listener.m) ---
% NOTE: This function's definition is only included here for completeness of the flow.
% The logic for this function must be implemented in a separate file for deployment!
...
%   NOTE: The ProjectFusion_Learning_Listener_m1 script is used for manual testing.
%   This definition is the core DEPLOYED logic.
    ...
    try
        ...
        if isfield(data_struct, 'documentUrl')
             ...
             try
                 % CRITICAL STEP: Downloads the document content from the web URL.
